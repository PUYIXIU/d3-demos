...
function setData(type, data) {
    return new Promise((resolve, reject) => {
        axios.post(baseUrl+'/saveData',{type,data}).then(res=>resolve())
    });
}

function setErrorData(data) {
    return new Promise((resolve, reject) => {
        axios.post(`${baseUrl}/errorData`).then(()=>resolve())
    });
}

function exportData() {
    axios.post(`${baseUrl}/export`).then(({data})=>{
...
function reset() {
    axios.post(`${baseUrl}/reset`).then(({data})=>{
        console.log("重置成功");
    })
}
